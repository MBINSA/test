...
def input_data(name_W, name_MY, name_D, name_BP, name_T):
    """
    ...
    # with open('02_Actions_performances.csv', 'w', encoding='UTF8', newline='') as AP_csv:
    #         writer = csv.writer(AP_csv)
    #         # write the first header corresponding to the actions
    #         writer.writerow(A)
    #         # write the data corresponding to the performances
    #         for a in A:
    #             writer.writerow(AP[a])

    return C, W, A, AP, B, BP, T

def MonteCarlo (MY, D):
    AP = {}
    for s in MY:                   #parcours les scenarios
        perf_scenar = {}
        for d in D:
            M = float(MY[s][d])
            if D[d] == 'N':
                if M != 0 : 
                    sig = 0.2*abs(M)
                    v = np.random.normal(M,sig,1)
                    perf_scenar[d] = v[0]
                else : 
                    perf_scenar[d] = M
            if D[d] == 'P':
                perf_scenar[d] = np.random.poisson(M, 1)
            if D[d] == 'E':
                lamb = 1/(M)
                perf_scenar[d] = np.random.exponential(lamb,1)
            if D[d] == 'LN':
                perf_scenar[d] = np.random.lognormal(M,225,1)
        AP[s] = perf_scenar
    return AP
